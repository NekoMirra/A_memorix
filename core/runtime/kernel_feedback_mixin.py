...
class KernelFeedbackMixin:
    ...
    def _feedback_noise(text: str) -> bool:
        ...
        if not content:
            return True
        if KernelFeedbackMixin._feedback_contains_signal(content):
            return False
        if len(content) <= 2:
            return True
        markers = (
    ...
    def _extract_feedback_messages(
        *,
        ...
        for item in raw_messages:
            ...
            if KernelFeedbackMixin._feedback_noise(text):
                continue
            if text in seen:
                continue
            seen.add(text)
    ...
    def _should_invoke_feedback_classifier(feedback_messages: List[str]) -> bool:
        ...
        return any(token in lowered for token in KernelFeedbackMixin._feedback_signal_tokens())
